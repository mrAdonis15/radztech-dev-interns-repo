...
export function isRawChartJsonText(text) {
  ...
  return (
    ...
    /\b"datasets"\s*:/.test(t) ||
    (/\b"data"\s*:/.test(t) &&
      (/\b"labels"\s*:/.test(t) || /\b"items"\s*:/.test(t))) ||
    /\b"graph"\s*:/.test(t) ||
    (/\b"config"\s*:/.test(t) &&
...
export function isLikelyChartData(payload) {
  ...
  if (
    payload.glChart &&
    (Array.isArray(payload.rep) || payload.rep === undefined)
  ) {
    return true;
  }
  ...
  if (Array.isArray(payload.rep) && payload.rep.length > 0) {
    ...
    const hasCredit =
      ...
      first.cr != null ||
      first.amountCr != null ||
      first.amount_cr != null ||
      first.credit != null ||
      first.Credit != null ||
      first.tCr != null;
    const hasBal =
      first.runBal != null ||
      first.runBalance != null ||
      first.balance != null ||
      first.Balance != null ||
    ...
    const hasDate =
      ...
      first.date != null ||
      first.Date != null ||
      first.transDate != null ||
      first.postDate != null ||
      first.dt != null;
    const hasPeriod =
      first.YrMo != null || first.YrWk != null || first.period != null;
    if ((hasDate || hasPeriod) && (hasDebit || hasCredit || hasBal))
      return true;
  }
  if (payload.chart?.series?.length && payload.chart?.xAxis?.categories)
    return true;
  if (payload.config?.data?.labels && payload.config?.data?.datasets?.length)
    return true;
  ...
  if (
    ...
    (payload.labels?.length || payload.xAxis?.categories?.length)
  )
    return true;
  if (
...
function getYearFromPeriod(p) {
  ...
  const w = s.match(/^\d{4}-W?(\d{1,2})$/i);
  return w
    ? getThursdayOfIsoWeek(y, parseInt(w[1], 10) || 1).getFullYear()
    : Number.isFinite(y)
      ? y
      : null;
}
...
function yearFromLabels(labels) {
  ...
  return Number.isFinite(parseInt(y, 10))
    ? parseInt(y, 10)
...
function getGLDebit(r) {
  if (r == null) return 0;
  const v =
    r.Dr ?? r.dr ?? r.amountDr ?? r.amount_dr ?? r.debit ?? r.Debit ?? r.tDr;
  return num(v);
}
function getGLCredit(r) {
  if (r == null) return 0;
  const v =
    r.Cr ?? r.cr ?? r.amountCr ?? r.amount_cr ?? r.credit ?? r.Credit ?? r.tCr;
  return num(v);
}
function getGLBalance(r) {
  if (r == null) return 0;
  const v =
    r.runBal ?? r.runBalance ?? r.balance ?? r.Balance ?? r.runningBalance;
  return num(v);
}
...
function getGLDateParts(r) {
  ...
  if (iso)
    return { year: parseInt(iso[1], 10), month: parseInt(iso[2], 10) || 1 };
  const slash = s.match(/(\d{4})[/-](\d{1,2})/);
  if (slash)
    return { year: parseInt(slash[1], 10), month: parseInt(slash[2], 10) || 1 };
  return null;
}

// —— Item field access (supports multiple API names, including GL summary tDr/tCr) ——
function getIn(i) {
  return num(i.tIN ?? i.in ?? i.In ?? i.tDr ?? i.Dr ?? i.debit ?? i.Debit);
}
function getOut(i) {
  return Math.abs(
    num(i.tOUT ?? i.out ?? i.Out ?? i.tCr ?? i.Cr ?? i.credit ?? i.Credit),
  );
}
function getBal(i) {
  return num(i.runBal ?? i.balance ?? i.Balance ?? i.runBalance);
}
...
function aggregateToMonthly(items, year) {
  const toUse =
    items.length > ITEMS_AGGREGATE_MAX
      ? items.slice(-ITEMS_AGGREGATE_MAX)
      : items;
  const sorted = [...toUse].sort((a, b) =>
    String(getPeriod(a)).localeCompare(String(getPeriod(b)), undefined, {
      numeric: true,
    }),
  );
  ...
  for (const i of sorted) {
    ...
    if (
      m == null ||
      (getYearFromPeriod(getPeriod(i)) != null &&
        getYearFromPeriod(getPeriod(i)) !== year)
    )
      continue;
    const idx = m - 1;
    inData[idx] += getIn(i);
...
function onePointPerRow(items) {
  ...
  const sorted = [...toUse].sort((a, b) => {
    ...
    const c = String(pa).localeCompare(String(pb), undefined, {
      numeric: true,
    });
    if (c !== 0) return c;
  ...
  return {
    ...
    datasets: [
      ...
      },
      {
        ...
        data: runBalData,
        borderColor: RUNBAL_COLOR,
        backgroundColor: "rgba(75, 192, 192, 0.1)",
        fill: true,
        tension: 0.3,
        pointRadius: 4,
        pointBackgroundColor: RUNBAL_COLOR,
  ...
  return {
    ...
    datasets: [
      ...
      },
      {
        ...
        label: "Running Balance",
        data: runBalData,
        borderColor: GL_RUNBAL_COLOR,
        backgroundColor: "rgba(34, 197, 94, 0.1)",
        fill: true,
        tension: 0.3,
        pointRadius: 4,
        pointBackgroundColor: GL_RUNBAL_COLOR,
        pointBorderColor: "#fff",
        pointBorderWidth: 1,
        order: 3,
      },
...
function makeGLChartWithLabels(
  labels,
  title,
  debitData,
  creditData,
...
function fromItemsFormat(apiData) {
  ...
  const year = resolveYear(
    apiData,
    valid.map((i) => getPeriod(i)),
    new Date().getFullYear(),
  );
  const agg = valid.length
    ? aggregateToMonthly(valid, year)
    : emptyMonthly(year);
  return makeStockCardChart(
    year,
    apiData.title,
    agg.inData,
    agg.outData,
...
function fromItemsFormatGL(apiData) {
  ...
  const years = [
    ...new Set(
      periods.map((p) => getYearFromPeriod(p)).filter(Number.isFinite),
    ),
  ];
...
function fromDataConfigFormat(apiData) {
  ...
  const year = resolveYear(
    { ...apiData, year: config.year },
    data.map((r) => r.period ?? r.YrWk ?? r.YrMo),
    new Date().getFullYear(),
  );
  ...
  const agg = data.length
    ? aggregateToMonthly(items, year)
    : emptyMonthly(year);
  return makeStockCardChart(
    year,
    config.title,
...
function fromChartSeriesFormat(apiData) {
  ...
  const yearFromReq =
    apiData.year ??
    (apiData.dt1 != null
      ? parseInt(String(apiData.dt1).slice(0, 4), 10)
      : null);
  const title =
    (Number.isFinite(yearFromReq) ? `YEAR ${yearFromReq}` : null) ||
    c.title ||
    "Stock Card Graph";
  return {
    chartType: "mixed",
    title,
    labels,
    datasets: c.series.map(seriesToDataset),
  };
}
...
function fromGenericFormat(apiData) {
  const labels = (apiData.labels || apiData.xAxis?.categories || []).map(
    formatLabel,
  );
  ...
  const datasets = series.map((s, i) => {
    ...
    const isLine =
      s.type === "line" || /running balance|balance|trend/i.test(label);
  ...
  return {
    chartType: "mixed",
    title: apiData.title || apiData.chart?.title || "Chart",
    labels,
    datasets,
  };
}
...
function fromGLRepFormat(apiData) {
  ...
  const repToUse =
    rep.length > GL_REP_MAX_ROWS ? rep.slice(-GL_REP_MAX_ROWS) : rep;
  const dateParts = repToUse.map((r) => getGLDateParts(r)).filter(Boolean);
  const year = resolveYear(
    apiData,
    dateParts.length
      ? dateParts.map((p) => `${p.year}-${String(p.month).padStart(2, "0")}`)
      : [],
    new Date().getFullYear(),
  );
  ...
  const getSortKey = (r) => {
    let d =
      r?.jDate ?? r?.date ?? r?.Date ?? r?.transDate ?? r?.postDate ?? r?.dt;
    if (d == null) {
      const p = r?.YrMo ?? r?.YrWk ?? r?.period;
      return p != null ? String(p).trim() : "";
    }
    if (typeof d === "number")
      d = new Date(d < 1e12 ? d * 1000 : d).toISOString();
  ...
  sortedIndices.sort((a, b) =>
    getSortKey(repToUse[a]).localeCompare(getSortKey(repToUse[b]), undefined, {
      numeric: true,
    }),
  );
  for (const i of sortedIndices) {
    ...
    if (!parts || parts.month < 1 || parts.month > 12 || parts.year !== year)
      continue;
...
function fromGLEmptyRepFormat(apiData) {
  ...
  const totalDr = num(
    apiData.tDr ?? apiData.totalDr ?? apiData.periodBalDr ?? apiData.tDrFA,
  );
  const totalCr = num(
    apiData.tCr ?? apiData.totalCr ?? apiData.periodBalCr ?? apiData.tCrFA,
  );
...
function fromAnyChartLike(payload) {
  ...
  if (
    ...
    labels = hasPeriod
      ? d.map((r) => getPeriod(r) || r.period || "")
      : d.map((_, i) => `${i + 1}`);
    series = hasPeriod
      ? [
          { name: "IN", data: d.map((r) => getIn(r)) },
          { name: "OUT", data: d.map((r) => getOut(r)) },
          { name: "Running Balance", data: d.map((r) => getBal(r)) },
        ]
      : [
          {
            name: "Value",
            data: d.map((v) =>
              num(typeof v === "object" ? (v.value ?? v.data) : v),
            ),
          },
        ];
  }
  ...
  if (!labels.length && series[0]?.data?.length)
    labels = series[0].data.map((_, i) => `${i + 1}`);
...
export function buildStockCardRunningBalanceChart(apiData) {
  ...
  const isGLResponse =
    apiData.glChart === true ||
    (Array.isArray(apiData.rep) &&
      (apiData.begAmt != null ||
        apiData.endAmt != null ||
        apiData.dt1 != null));
  if (isGLResponse && repArray.length === 0) {
    const builtEmpty = fromGLEmptyRepFormat({
      ...apiData,
      rep: repArray,
      glChart: true,
    });
  ...
  if (Array.isArray(rep) && rep.length > 0) {
    ...
    if (hasPeriod && (hasDebit || hasCredit || hasBal) && isGLResponse) {
      ...
      const builtFromRep = fromItemsFormatGL({
        ...apiData,
        items: itemsFromRep,
      });
      if (builtFromRep) return builtFromRep;
    }
    if (hasDate && (hasDebit || hasCredit || hasBal))
      return fromGLRepFormat({ ...apiData, rep });
  ...
  if (isGLResponse && Array.isArray(items))
    return fromItemsFormatGL({ ...apiData, items });
  if (Array.isArray(apiData.items)) return fromItemsFormat(apiData);
  if (Array.isArray(items)) return fromItemsFormat({ ...apiData, items });
  if (Array.isArray(apiData.data) && apiData.config)
    return fromDataConfigFormat(apiData);
  if (apiData.chart?.series?.length) {
    const c = apiData.chart;
    if (!c.xAxis?.categories?.length && c.series?.[0]?.data?.length)
      return fromChartSeriesFormat({
        ...apiData,
        chart: {
          ...c,
          xAxis: { categories: c.series[0].data.map((_, i) => `${i + 1}`) },
        },
      });
    if (c.xAxis?.categories?.length) return fromChartSeriesFormat(apiData);
  }
  if (apiData.config?.data?.labels && apiData.config?.data?.datasets)
    return fromConfigDataFormat(apiData);
  if (
    (apiData.series || apiData.datasets)?.length &&
    (apiData.labels || apiData.xAxis?.categories)
  )
    return fromGenericFormat(apiData);
  return null;
}
