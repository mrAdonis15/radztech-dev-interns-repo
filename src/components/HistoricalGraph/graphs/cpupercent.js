...
function getHistoricalChart(data, intervalMs) {
  ...
  raw.forEach(function (d, i) {
    const t = d.timestamp != null ? d.timestamp : now - (raw.length - 1 - i) * interval;
    if (t >= cutoff) {
      ...
      memUsed.push(d.mem && d.mem.used_percent != null ? d.mem.used_percent : 0);
  ...
  const currentMemUsedKb = lastMem && lastMem.used_kb != null ? lastMem.used_kb : 0;
  const currentMemTotalKb = lastMem && lastMem.total_kb != null ? lastMem.total_kb : 0;
      ...
      yAxes: [
        {
          ...
          ticks: { fontColor: '#666', beginAtZero: true, max: 100, fontSize: 11, stepSize: 25 },
        },
      ],
    },
  };

  return { chartData, chartOptions, currentCpu, currentMem, currentMemUsedKb, currentMemTotalKb };
}


function CpuPercentHistorical({ data: dataProp, intervalMs = 1000, height = 320 }) {
  const [localData, setLocalData] = useState([]);
  const [dataIndex, setDataIndex] = useState(0);
  const data = localData;
  const { chartData, chartOptions } = getHistoricalChart(data, intervalMs);
  const maxDataPoints = SAMPLE_DATA.length;
  ...
  useEffect(() => {
    ...
    if (chartRef.current) {
      try {
        ...
        chartRef.current = new Chart(ctx, { type: 'line', data: chartData, options: chartOptions });
      }
    } else {
      chartRef.current = new Chart(ctx, { type: 'line', data: chartData, options: chartOptions });
    }
  ...
  return (
    ...
    >
      <Box px={2} pt={2} pb={0} display="flex" alignItems="baseline" justifyContent="space-between" flexWrap="wrap">
        ...
        <div>
          <Button variant="contained" color="primary" size="small" onClick={addDataPoint} disabled={isMax}>
...
export { getHistoricalChart, CpuPercentHistorical, padToThreeMinutes, SAMPLE_DATA };
